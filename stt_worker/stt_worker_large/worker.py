...
consumer = create_kafka_consumer('stt_whisper_large_worker_group', ['audio_requests'])
...
if __name__ == '__main__':
    ...
    logging.info("Whisper Large STT Worker is running and waiting for tasks...")
    for message in consumer:
        task = message.value
        
        # 이 워커는 'large-v3' 모델만 처리
        if task.get("model") != "large-v3" or task.get("diarization", False):
            continue
            ...
            with tempfile.NamedTemporaryFile(delete=True, suffix=".tmp") as original_audio_file:
                ...
                for i in range(0, int(duration), CHUNK_LENGTH_S):
                    with tempfile.NamedTemporaryFile(delete=True, suffix=".mp3") as chunk_file:
                        ...
                        subprocess.run(cmd, True, capture_output=True)
